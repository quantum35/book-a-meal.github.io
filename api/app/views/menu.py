...
from  app.views.auth.validate import require_admin, space_stripper, name_validator, num_check
...
class MenuList(Resource):
    ...
    def post(self):
        ...
        if space_stripper(menu_item) == '':
            return {"status":"Failed",
            "message":"Meal name can not be empty.Please enter a valid menu item"},406
        if meal_id == '' :
            return {"status":"Failed",
            "message":"Meal id can not be empty.Please enter a valid meal id"},406
        if  meal is None:
